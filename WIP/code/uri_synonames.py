...
df = pd.read_sql_query('select * from names limit 1000',con=conn)
#df.to_csv("test_data/test_data_500k.csv", index = False)
#df = pd.read_csv("test_data_300k.csv")
print (df.head())

#Clean data 
sup_lang = ['bg', 'el', 'hy', 'ka', 'l1', 'mk', 'mn', 'ru', 'sr-ec','uk', 'sr', 'ast', 'ca', 'cy', 
'da','de','en','es','fi','fr','hu','it','nb','nl', 'ms', 'ro', 'nn', 'sl','sq','sv', 'vec', 'pl',
 'pt', 'bg', 'cs', 'vi', 'gd', 'tr', 'et', 'lt', 'sl']
df = df[df.lang.isin(sup_lang)]

df = df.drop_duplicates(subset = ["uri", "lang"], keep = 'first') #One ref per language per uri 
...
#names_melt["phonetics"] = names_melt["phonetics"].apply(lambda x: print (x[1]))
...
print ("pattern_temp", pattern_temp.head())
del stack_df 
...
print ("pattern_df", pattern_df.head())
del pattern_temp
...
phon_group.to_csv("names_nodes_neo4j.csv", index = False)
phon_group["stack_value"] = phon_group["stack_value"].apply(lambda x: ','.join(x))
phon_group.to_csv("output/names_nodes_str_neo4j.csv", index = False)

phon_group["check"] = False  #flag problem uris
phon_group.loc[phon_group["uri"].isin(sim_uris), "check"] = True 
phon_group.to_csv("output/names_nodes_str_neo4j_flagged_uris.csv", index = False)
